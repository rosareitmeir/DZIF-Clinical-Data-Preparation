...
#### After Event
```{r}
crea_afterevent<-  ggplot(creatinine_plotdf %>% filter(rej==1 & Ascertainment_Event_diffs >=0  & PatientID != "1G6AH9R7") %>% mutate(int= case_when(PatientID %in% c("P6HX9AK1", "PCUXMQR2","PY2KYQ34","VPK5WL27") ~ "yes", T ~ "no" )), 
         aes(x=as.numeric(Ascertainment_Event_diffs), y=CreatinineFinal)) + 
            geom_line(aes(group=PatientID, ), linetype="dashed") + 
            geom_point(aes(color=type, shape=int), size = 3, shape = 16) + 
              theme(legend.position = "none") + scale_color_manual(values = coloring) +
              geom_smooth() + 
              xlab("Day Distance to Event")+ ylab("Creatinine in mg/dL")+  theme_bw() + 
   theme(
    strip.text = element_text(size=16),
    strip.background = element_rect(fill="white", linewidth = 0),
